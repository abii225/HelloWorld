...
interface ProfileModalProps {
  ...
  return (
    <>
      ...
      <h2 style={{textAlign:"center",marginBottom:"20px",fontSize:"20px",color:"green"}}>Profile Information</h2>

        {
          isEditing ?
            <div>
               <div className="flex justify-center mb-3">
                <img className="w-24 h-24 p-1 m-3 rounded-full ring-2 ring-gray-300 dark:ring-gray-500 " src={profile ? profile : "https:static.vecteezy.com/system/resources/previews/020/765/399/non_2x/default-profile-account-unknown-icon-black-silhouette-free-vector.jpg"} alt="Bordered avatar" />
               </div>
              <div className="relative z-0 w-full mb-6 group">
                <input type="text" name="floating_email" id="floating_email" 
                  value={username} onChange={(e) => setUpdateUserData({...updateUserData,['username']:e.target.value})}
              ...
              <div className="relative z-0 w-full mb-6 group">
                <input type="password" name="floating_password" id="floating_password" 
                  value={password} onChange={(e) => setUpdateUserData({...updateUserData,['password']:e.target.value})}
              ...
              <div className="relative z-0 w-full mb-6 group">
                <input type="password" name="repeat_password" id="floating_repeat_password"
                  value={confirm_password} onChange={(e) => setUpdateUserData({...updateUserData,['confirm_password']:e.target.value})}
                   className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required />
                <label className="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Confirm password</label>
              </div>
              <div className="relative z-0 w-full mb-6 group">
                <input type="text" name="floating_email" id="floating_email" 
                   value={bio} onChange={(e) => setUpdateUserData({...updateUserData,['bio']:e.target.value})}
                className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required />
              ...
              <div className="flex justify-between">
                <button onClick={()=> setIsEditing(false)} className="btn mt-3">Back</button>
                <button onClick={handleSaveClick} className="btn mt-3">Update</button>
              </div>
            </div>
            : <div>
              <div className="flex justify-center mb-3">
              ...
              <div className="flex justify-around ">
                ...
                <p className="mb-1"> Username :-{` ${loggedInUser.username}`}</p>
                <p>Bio :- {`${loggedInUser.bio}`}</p>
                </div>
                <button onClick={()=> setIsEditing(true)} className="btn mt-2 ">Edit Profile</button>
              </div>
          </div> 
        } 
</div>
